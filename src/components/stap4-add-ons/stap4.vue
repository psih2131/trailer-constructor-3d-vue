<template>
    <div class="body-main-data__wrapper" v-if="store.dataServer['add-ons']?.stap_4?.length >0">
...
const selectCurrentSize = (item, index)=>{
    ...
    store.stapsMemory.stap4_AddOns.stap4.selectedElements = valueStoreArray
...
onMounted(()=>{
    ...
    sizeList.value = store.dataServer['add-ons'].stap_4
    ...
    if(store.stapsMemory.stap4_AddOns.stap4.selectedElements.length > 0){

        let arraySelectedListStore = store.stapsMemory.stap4_AddOns.stap4.selectedElements
        ...
        store.stapsMemory.stap4_AddOns.stap4.selectedElements = []
    }
