...
const selectCurrentSize = (item, index)=>{
    ...
    if(props.typeSelect == 'one'){
        ...
        for(let i = 0; i < arrayListObject.value.length; i++){
            if(+i != +index){
                arrayListObject.value[i].selected = false
                console.log('gg')
            }
            console.log(arrayListObject.value[i].selected, i, index)
        }


        if(arrayListObject.value[index].selected == true){
            let object = {
                'currentIndex': +index,
